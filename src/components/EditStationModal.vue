<template>
  <q-dialog :v-model="showModal">
    <q-card style="min-width: 40%">
      <h5 class="title">Editar Estação <q-icon name="store" /> </h5>
...
export default {
  ...
  props: { station: Object, showModal: Boolean },

  data() {
    return {
      img: this.station.img,
      address: this.station.address,
      distance: this.station.distance,
    };
  },
  emits: ["edit"],
  methods: {
    edit() {
      const json = {
        id: this.station.id,
        address: this.address ?? this.station.address,
        img: this.img ?? this.station.img,
        distance: this.distance ?? this.station.distance,
      };
      this.$emit("edit", json);
    },
  },
